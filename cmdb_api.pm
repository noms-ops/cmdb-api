...
sub doFieldNormalization()
{
    ...
    $value =~ s/\ $//g if defined $value;
    my $matchers = $dbh->selectall_arrayref( 'select matcher,sub_value from inv_normalizer where entity_name=? and field_name=?', {}, ( $entity, $field ) );
    foreach (@$matchers)
    {
...
sub doEnvironmentsServicesGET()
{
    ...
    {
        $sql .= sprintf "and %s %s '%s' ", $key, $getparams{$key}{op}, $getparams{$key}{val};
    }

    $sql .= "and environment_name in ($list)) as s " . "left join service_instance_data as d on s.svc_id = d.svc_id " . "order by field(environment_name, $list)";
    $rtn = &doSql( $sql, undef );
...
sub doEnvironmentsServicesPUT()
{
    my $requestObject = shift;
    $logger->info("processing PUT");
    my $dbh             = DBI->connect( "DBI:$DRIVER:database=$DATABASE;host=$DBHOST", $DBUSER, $DBPASS, { AutoCommit => 0, RaiseError => 1 } );
    ...
    eval {
        ...
        $sql = "select data_id,data_key,data_value " . "from service_instance_data where svc_id=?";
        $sth = $dbh->prepare($sql);
        ...
        for my $key ( keys %$data )
        {
            ...
            if ( exists $service_attributes{$key} )
            {
                ...
                elsif ( $service_attributes{$key}[0] ne $data->{$key} )
                {
                    push @updates, $key;
                }
            }
        ...
        if (@inserts)
        {
            ...
            $sql = "insert into service_instance_data " . "(data_key, data_value, svc_id) values ";
            $sql .= join( ',', map { sprintf( "('%s','%s','%s')", $_, $data->{$_}, $svc_id ) } @inserts );
        ...
        if (@updates)
        {
            ...
            $sql = "update service_instance_data set " . "data_value=? where data_key=? and svc_id=?";
            $sth = $dbh->prepare($sql);
        ...
        if (@deletes)
        {
            $sql = "delete from service_instance_data where " . "svc_id=? and data_key in ";
        ...
        if ($did_update)
        {
            insertAuditEntry( $dbh, $requestObject, 'services', "$environment/$service", 'record', make_json($old_value), make_json($new_value), $$now[0] );
            $dbh->commit;
        }
    };
    if ($@)
...
sub doEnvironmentsServicesPOST()
{
    ...
    eval {
        ...
        $sql = sprintf( "insert into service_instance (%s) values (%s)", join( ', ', @columns ), join( ', ', map { "'$_'" } @values ) );
        $sth = $dbh->prepare($sql);
        executeDbStatement( $sth, $sql );
...
sub doEnvironmentsServicesDELETE()
{
    ...
    eval {
        ...
        insertAuditEntry( $dbh, $requestObject, 'services', "$environment/$service", 'record', make_json($old_value), 'DELETED', $$now[0] );
        $dbh->commit;
    };
    if ($@)
    {
    ...
    foreach (@$meta_fields)
    {
        ...
        else
        {
            ...
            if ( defined $getparams{$_} )
            {
                ...
                # add a or is null as NOT matches will be true for null outer joins
                elsif ( $getparams{$_}{op} =~ m/NOT/ )
                {
                    $where_sql .= " ( m$x.metadata_value $op ?";
                    $where_sql .= " OR m$x.metadata_value is null)";
                }
                else
                {
                    $where_sql .= " m$x.metadata_value $op ?";
...
sub doSystemPUT()
{
    ...
    if (
        ( $data->{$IPADDRESSFIELD} && $data->{$IPADDRESSFIELD} ne $lkup_data->{$IPADDRESSFIELD} )
        || (
